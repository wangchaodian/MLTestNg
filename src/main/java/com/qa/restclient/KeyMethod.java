...
public class KeyMethod {
	...
	public static String getParam; // get请求参数
...
//					Excel中url列
					...
					if (tableName.equals("post")) {
						...
						try {
							getParam = BodyHead.st(parameter);
						} catch (Exception e) {
							getParam = parameter;
						}
						getActions(tableName, dns+url+getParam);
					}
	...
	//post请求
	public static void postActions(String tableName,String InterfaceUrl) throws Exception{
		...
		if (parameter.contains("{")) {
			...
			headermap = BodyHead.headMap("application/json; charset=utf-8", token);
		}else {
			headermap = BodyHead.headMap(" application/x-www-form-urlencoded", token);
		}
...
//	get请求方法发送
	public static void getActions(String tableName,String InterfaceUrl) throws Exception{
...
//		判断响应结果是否包含期望结果
		...
		try {
			...
			System.out.println(sTestCaseID+":响应正文不是json");
			e.printStackTrace();
		}
